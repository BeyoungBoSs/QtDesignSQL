...
from PySide2.QtWidgets import QMessageBox, QInputDialog, QPlainTextEdit
...
class Manifest(QtWidgets.QMainWindow):
    ...
    def __init__(self, uid, filename):
        ...
        # 发送按钮
        # if self.ui.test_edit.keyPressEvent():
        #     self.send_files()
        ...
        try:
            while 1:
                ...
                if Manifest.targetUid != -1:
                    sql = "select id,message,time,uidSend from message where ( uidSend = '" + str(
                        Manifest.gUid) + "' and uidRecipient = '" + str(
                        Manifest.targetUid) + "' ) or ( uidSend = '" + str(
                        Manifest.targetUid) + "' and uidRecipient = '" + str(Manifest.gUid) + "' )"
                    print(sql)
                ...
                if Manifest.targetUid != -1:
                    ...
                    for i in re:
                        ...
                        if i[0] not in Manifest.list:
                            ...
                            print("]")
                            time.sleep(0.05)
                            self.ui.messages.append(ss + str(i[2]) + s_last + ss + i[
                                1] + s_last + '<p><font color="black">-------------------------------</font></p>')
                            print("!")
        ...
        for uid in s:
            ...
            for flag in re3:
                if uname not in list:
                    if flag[0] == 1 and ((flag[1] == Manifest.gUid and flag[2] == uid) or (
                            flag[2] == Manifest.gUid and flag[1] == uid)):
        ...
        if s == "":
            ...
            if s1 != s:
                ...
                if re == QMessageBox.Yes:
                    sql = 'update contact set flag=1 where ( uid1 = "' + str(Manifest.gUid) + '" and uid2 = "' + str(
                        i) + '" ) or ( uid1 = "' + str(i) + '" and uid2 = "' + str(Manifest.gUid) + '" ) '
                    SqlCorporate.insert(sql)
                else:
                    re1 = showMessage(self.contactList, "注意", '不成为好友，是否删除好友请求" ' + s1 + ' "?',
                                      QMessageBox.Yes | QMessageBox.No)
                    if re1 == QMessageBox.Yes:
                        sql = 'DELETE FROM contact WHERE ( uid1 = "' + str(Manifest.gUid) + '" and uid2 = "' + str(
                            i) + '" ) or ( uid1 = "' + str(i) + '" and uid2 = "' + str(Manifest.gUid) + '" ) '
                        SqlCorporate.insert(sql)
                self.refresh()
            else:
    ...
    def send(self):
        ...
        if strings == "":
            ...
            return
        sql = "INSERT INTO message(uidSend, uidRecipient, message, time) VALUES ('%d', '%d',  '%s',  '%s')" % (
        Manifest.gUid, Manifest.targetUid, strings, dt)
        SqlCorporate.insert(sql)
    ...
    def display(self):
        ...
        SqlCorporate.insert(
            'CREATE VIEW mess AS SELECT users.uname,message.message FROM message,users WHERE message.uidSend = ' + str(
                Manifest.gUid) + ' and users.uid=message.uidSend')
    ...
    def history_display(self, flag, s):
        ...
        if flag == 1:
            sql = "select id,message,time,uidSend,uidRecipient from message where ( uidSend = '" + str(
                Manifest.gUid) + "' and uidRecipient = '" + str(s) + "' ) or ( uidSend = '" + str(
                s) + "' and uidRecipient = '" + str(Manifest.gUid) + "' )"
        elif flag == 2:
            print(2)
            ...
            sql = "select id,message,time,uidSend,uidRecipient from message where  uidSend = '" + str(
                Manifest.gUid) + "' or  uidRecipient = '" + str(Manifest.gUid) + "' "
        print(sql)
        re = SqlCorporate.execute(sql)
        print(re)
        for i in re:
            ...
            # time.sleep(0.1)
            print(222)
            self.search.text.append(ss + u2u + str(i[2]) + s_last + ss + i[
                1] + s_last + '<p><font color="black">-------------------------------</font></p>')
